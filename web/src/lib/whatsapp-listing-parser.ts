...
function extractPrice(text: string): { value: number; rentPeriod?: 'day' | 'month' | 'year'; rest: string } {
  let rest = text;
  let value = 0;
  ...
  // ₦5m, 5m, 5 million, 500k, 500,000, 5.5m
  const patterns = [
    /(?:₦|N|NGN)\s*([\d.,]+)\s*(m|million|k|thousand)?/i,
    /([\d.,]+)\s*(m|million|k|thousand)\b/i,
    /(?:₦|N|NGN)\s*([\d.,]+)/i,
    /(?:price|#|amount)\s*[:\s]*([\d.,]+)\s*(m|million|k|thousand)?/i,
  ];
...
function extractPhone(text: string): { phone: string | undefined; rest: string } {
  const re = /(?:\b|call|contact|whatsapp|whatsapp:)\s*([0]?\d{10,11}|[\+]234\s*\d{10})\b/gi;
...
function extractLocation(text: string): { state: string; city: string; address: string; suburb?: string; rest: string } {
  ...
  address = [suburb || city, validState].filter(Boolean).join(', ');
...
export function parseWhatsAppListingText(raw: string): ParseResult {
  ...
  const { value: price, rentPeriod, rest: r1 } = extractPrice(text);
