...
export function AdminTrendSocialPostButtons({
  trendId,
  facebookPostId,
  instagramPostId,
  instagramPermalink,
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/admin/trends/${trendId}/social-post`)
  ...
  async function post(platform: SocialPlatform) {
    ...
    if (facebookAgain || twitterAgain) {
      ...
      if (!window.confirm(`Already posted to ${parts.join(' and ')}. Post again?`)) return;
    }

    setBusy(platform);
    try {
      const res = await fetch(`/api/admin/trends/${trendId}/social-post`, {
        method: 'POST',
      ...
      if (res.status === 409 && data.alreadyPosted) {
        if (!window.confirm(`${data.error || 'Already posted.'} Post again?`)) return;
        const retry = await fetch(`/api/admin/trends/${trendId}/social-post`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ platform, force: true }),
        });
        applyResult(retry, (await retry.json().catch(() => ({}))) as typeof data);
        return;
      }

      applyResult(res, data);
    } catch {
      alert('Failed to post trend');
    } finally {
      setBusy(null);
    }
  }
  ...
  function applyResult(
    ...
    if (!res.ok && lines.length === 0) {
      alert(typeof data.error === 'string' ? data.error : 'Failed to post trend');
      return;
    }
    if (lines.length) alert(lines.join('\n'));
    else if (!res.ok) alert(typeof data.error === 'string' ? data.error : 'Failed to post trend');
  ...
  return (
    <div className={compact ? 'inline-flex flex-wrap items-center gap-1' : 'mt-3 space-y-2'}>
      {!compact ? <h4 className="text-sm font-medium text-gray-900">Post to social</h4> : null}
      <button
        type="button"
        onClick={() => post('facebook')}
        disabled={busy !== null || !facebookOn}
        title={facebookOn ? 'Publish this trend to Facebook Page and Instagram' : 'Configure Facebook Page env vars'}
      ...
      <button
        ...
        title={twitterOn ? 'Publish this trend to X' : 'Configure X API env vars'}
