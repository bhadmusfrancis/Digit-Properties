...
type TrendsContentProps = {
  initialPosts?: TrendPost[];
  initialPagination?: { page: number; pages: number };
};

function TrendsContent({ initialPosts = [], initialPagination }: TrendsContentProps) {
  ...
  const { data, isLoading } = useQuery({
    ...
    staleTime: 60 * 1000,
    initialData: initialPosts.length && !query.includes('category') && !query.includes('page=')
      ? { posts: initialPosts, pagination: initialPagination ?? { page: 1, pages: 1 } }
      : undefined,
  });
  const posts: TrendPost[] = data?.posts ?? initialPosts;
  const pagination = data?.pagination ?? initialPagination ?? { page: 1, pages: 1 };
...
export function TrendsPageClient(props: TrendsContentProps) {
  return (
