...
The app can send a **6-digit OTP to the user’s WhatsApp number** and verify the code they enter. This uses **Twilio Verify API** with channel `whatsapp`.
...
- If these are set, the app **prefers WhatsApp OTP** (Twilio Verify) for phone verification.
...
- **Verify phone (send code):**  
  ...
  If Twilio is configured, it sends the OTP to that number via WhatsApp and returns e.g.  
  `{ "ok": true, "message": "Verification code sent to your WhatsApp. Enter the code below.", "channel": "whatsapp" }`.
